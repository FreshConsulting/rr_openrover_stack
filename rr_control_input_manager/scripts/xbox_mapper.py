...
wired_or_wireless = rospy.get_param("/xbox_mapper_node/wired_or_wireless", "wired")
...
def joy_cb(Joy):
    ...
    cmd_time = float(Joy.header.stamp.secs) + (
        float(Joy.header.stamp.nsecs) / 1000000000
    )
    rbt_time = time.time()
    ...
    turn_cmd = (
        (1.1 - (drive_cmd / MAX_VEL_FWD))
        * DRIVE_THROTTLE
        * MAX_VEL_TURN
        * Joy.axes[R_STICK_H_AXES]
    )  # right joystick
